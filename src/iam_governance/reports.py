...
def export_markdown(
    campaign: dict[str, Any], sod_results: list[dict[str, Any]] | None = None
) -> Path:
    ...
    # Title
    lines.append("# Access Review Evidence Report")
    lines.append("")
    ...
    if sod_results:
        ...
        lines.append("## Segregation of Duties (SoD) Summary")
        lines.append("")
        if violators:
            lines.append(
                f"> **WARNING:** {len(violators)} user(s) have SoD conflicts "
                "that require immediate remediation."
            )
            lines.append("")
            lines.append("| User ID | Name | Critical | High |")
            lines.append("|---------|------|----------|------|")
            for v in violators:
                lines.append(
                    f"| {v['user_id']} | {v.get('name', '')} | "
                    f"{v['critical_count']} | {v['high_count']} |"
                )
        else:
            lines.append("> No SoD violations detected across all reviewed users.")
        lines.append("")
    ...
    if denied_items:
        ...
        lines.append("")
        lines.append("The following access items were **DENIED** and must be revoked:")
        lines.append("")
        lines.append("| User | Department | Application | Reviewer | Justification |")
        lines.append("|------|-----------|-------------|----------|---------------|")
    ...
    # Full evidence table
    lines.append("## Full Evidence - All Review Items")
    lines.append("")
    lines.append(
        "| Item ID | User | Role | Department | Application | Decision | Reviewer | Timestamp |"
    )
    lines.append(
        "|---------|------|------|-----------|-------------|----------|----------|-----------|"
    )
    ...
    lines.append("")
    lines.append("---")
    lines.append("*Report generated by IAM Governance Lab - for internal compliance use only.*")
